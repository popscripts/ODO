...
export function useClassModalSettings(classroom: Classroom, userData: User) {
    ...
    if (isDifferentClassroomTakenByMe(classroom, userData))
        settings.taken.disabled = true

    if (isClassroomTakenByMe(classroom, userData))
        settings.reserved.disabled = true

    if (isClassroomReserved(classroom, userData))
        settings.reserved.disabled = true

    if (isClassroomReserved(classroom, userData)) settings.taken.disabled = true

    if (isDifferentClassroomReservedByMe(classroom, userData))
        settings.reserved.disabled = true

    if (
        isClassroomReservedByMe(classroom, userData) &&
        !isDifferentClassroomTakenByMe(classroom, userData)
    )
        settings.taken.disabled = false

    if (isClassroomTakenByMe(classroom, userData))
        settings.free.disabled = false

    if (isClassroomReservedByMe(classroom, userData))
        settings.free.disabled = false

    if (
        isClassroomReservedByMe(classroom, userData) &&
        isClassroomTaken(classroom, userData)
    )
        settings.taken.disabled = true

    if (!isMemeberOfGroup(userData))
        settings = {
            taken: { disabled: true },
...
function isDifferentClassroomReservedByMe(
    classroom: Classroom,
    userData: User
) {
